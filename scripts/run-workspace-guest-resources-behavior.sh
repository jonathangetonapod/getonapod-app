...
readonly BEHAVIOR_TEST='supabase/tests/20260721_workspace_guest_resources_behavior.sql'
...
readonly -a REQUIRED_ENV=(
  ...
  'PGPASSWORD'
  'PGSSLMODE'
  'BEHAVIOR_DB_ENVIRONMENT'
  'BEHAVIOR_DB_EXPECTED_PGHOST'
  'BEHAVIOR_DB_EXPECTED_PROJECT_REF'
  'BEHAVIOR_DB_PRODUCTION_PGHOSTS'
  'BEHAVIOR_DB_CONFIRM'
)
...
refuse() {
  printf '%s\n' \
    'Workspace guest-resource behavior runner refused: target or source-integrity check failed.' \
    >&2
  exit 1
}
...
while IFS='=' read -r variable_name _; do
  case "${variable_name}" in
    BEHAVIOR_DB_ENVIRONMENT|BEHAVIOR_DB_EXPECTED_PGHOST|BEHAVIOR_DB_EXPECTED_PROJECT_REF|BEHAVIOR_DB_PRODUCTION_PGHOSTS|BEHAVIOR_DB_CONFIRM)
      ;;
    BEHAVIOR_DB_*)
      refuse
      ;;
...
[[ "${BEHAVIOR_DB_ENVIRONMENT}" == 'local' \
  || "${BEHAVIOR_DB_ENVIRONMENT}" == 'staging' ]] || refuse
validate_hostname "${PGHOST}" || refuse
validate_hostname "${BEHAVIOR_DB_EXPECTED_PGHOST}" || refuse
readonly canonical_pg_host="${PGHOST,,}"
readonly canonical_expected_host="${BEHAVIOR_DB_EXPECTED_PGHOST,,}"
readonly canonical_pg_user="${PGUSER,,}"
readonly canonical_pg_database="${PGDATABASE,,}"
readonly canonical_expected_project_ref="${BEHAVIOR_DB_EXPECTED_PROJECT_REF,,}"
...
if [[ "${BEHAVIOR_DB_ENVIRONMENT}" == 'local' ]]; then
...
IFS=',' read -r -a production_hosts <<< "${BEHAVIOR_DB_PRODUCTION_PGHOSTS}"
...
readonly expected_confirmation="RUN_ROLLBACK_BEHAVIOR_ON_${BEHAVIOR_DB_ENVIRONMENT}_${canonical_expected_project_ref}_${canonical_pg_host}:${PGPORT}/${PGDATABASE}?user=${PGUSER}@${release_commit}"
[[ "${BEHAVIOR_DB_CONFIRM}" == "${expected_confirmation}" ]] || refuse

scratch_dir="$(mktemp -d --tmpdir '.goap-wgr-behavior.XXXXXXXX')" || refuse
...
set +e
PGOPTIONS="-c goap.workspace_guest_resources_behavior=nonproduction-rollback-v1 -c goap.environment=${BEHAVIOR_DB_ENVIRONMENT} -c default_transaction_read_only=off -c statement_timeout=1140000 -c lock_timeout=30000" \
...
if (( behavior_exit != 0 )); then
  printf '%s\n' \
    'Workspace guest-resource behavior verification: FAILED (database output discarded).' \
    >&2
  exit 1
fi

printf '%s\n' \
  'Workspace guest-resource behavior verification: PASS (all mutations rolled back).'
