...
async function searchPodcasts(page: number = 1): Promise<PodscanSearchResponse> {
  const params = new URLSearchParams({
    category_ids: TOP_10_CATEGORIES.join(','),
    language: 'en',
    min_audience_size: '2000', // Lowered from 3000 to get more results
...
async function scrapePodcasts() {
  console.log('🚀 Starting podcast scraping - TOP 10 CATEGORIES')
  console.log(`📅 Filtering podcasts posted within last 100 days (since ${get100DaysAgo()})`)
  console.log(`🎯 Target: 700-1000 podcasts from top 10 categories`)
  console.log(`📊 Filters: English, min 2000 audience size`)
  console.log(`📂 Categories: Business, News, Culture, Society, Technology, Education, Comedy, Arts, History, Government\n`)
