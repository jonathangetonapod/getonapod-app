...
readonly BEHAVIOR_TEST='supabase/tests/20260722_subagency_workspace_foundation_behavior.sql'
...
readonly -a REQUIRED_ENV=(
  ...
  'PGPASSWORD'
  'PGSSLMODE'
  'FOUNDATION_DB_ENVIRONMENT'
  'FOUNDATION_DB_EXPECTED_PGHOST'
  'FOUNDATION_DB_EXPECTED_PROJECT_REF'
  'FOUNDATION_DB_PRODUCTION_PGHOSTS'
  'FOUNDATION_DB_CONFIRM'
)
...
refuse() {
  printf '%s\n' \
    'Sub-agency foundation behavior runner refused: target or source-integrity check failed.' \
    >&2
  exit 1
}
...
while IFS='=' read -r variable_name _; do
  case "${variable_name}" in
    FOUNDATION_DB_ENVIRONMENT|FOUNDATION_DB_EXPECTED_PGHOST|FOUNDATION_DB_EXPECTED_PROJECT_REF|FOUNDATION_DB_PRODUCTION_PGHOSTS|FOUNDATION_DB_CONFIRM)
      ;;
    FOUNDATION_DB_*)
      refuse
      ;;
...
[[ "${FOUNDATION_DB_ENVIRONMENT}" == 'local' \
  || "${FOUNDATION_DB_ENVIRONMENT}" == 'staging' ]] || refuse
validate_hostname "${PGHOST}" || refuse
validate_hostname "${FOUNDATION_DB_EXPECTED_PGHOST}" || refuse
readonly canonical_pg_host="${PGHOST,,}"
readonly canonical_expected_host="${FOUNDATION_DB_EXPECTED_PGHOST,,}"
readonly canonical_pg_user="${PGUSER,,}"
readonly canonical_pg_database="${PGDATABASE,,}"
readonly canonical_expected_project_ref="${FOUNDATION_DB_EXPECTED_PROJECT_REF,,}"
...
if [[ "${FOUNDATION_DB_ENVIRONMENT}" == 'local' ]]; then
...
IFS=',' read -r -a production_hosts <<< "${FOUNDATION_DB_PRODUCTION_PGHOSTS}"
...
readonly expected_confirmation="RUN_ROLLBACK_FOUNDATION_BEHAVIOR_ON_${FOUNDATION_DB_ENVIRONMENT}_${canonical_expected_project_ref}_${canonical_pg_host}:${PGPORT}/${PGDATABASE}?user=${PGUSER}@${release_commit}"
[[ "${FOUNDATION_DB_CONFIRM}" == "${expected_confirmation}" ]] || refuse

scratch_dir="$(mktemp -d --tmpdir '.goap-foundation-behavior.XXXXXXXX')" || refuse
...
set +e
PGOPTIONS="-c goap.subagency_foundation_behavior=nonproduction-rollback-v1 -c goap.environment=${FOUNDATION_DB_ENVIRONMENT} -c default_transaction_read_only=off -c statement_timeout=1140000 -c lock_timeout=30000" \
...
if (( behavior_exit != 0 )); then
  printf '%s\n' \
    'Sub-agency foundation behavior verification: FAILED (database output discarded).' \
    >&2
  exit 1
fi

printf '%s\n' \
  'Sub-agency foundation behavior verification: PASS (all mutations rolled back).'
