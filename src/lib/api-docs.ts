...
export const API_CATEGORIES: ApiCategory[] = [
  ...
  },
  {
    ...
    description: "Autoresearch tools for evaluating and tuning podcast-prospect vector search parameters against human feedback data.",
    endpoints: [
      {
        ...
        description: "Runs a single vector search experiment against all prospects with 10+ feedback entries. Compares search_similar_podcasts results against human-approved/rejected feedback to calculate precision, recall, and F1 score. Does NOT use AI scoring — tests vector search stage only.",
        auth: "API Key",
        notes: "Generates embeddings for each qualified prospect, runs vector search with given parameters, and compares predictions against the prospect_podcast_feedback table. Results are saved to matching_experiments table.",
        params: [
          { name: "similarity_threshold", type: "number", required: false, description: "Cosine similarity threshold for vector search (default: 0.30)" },
          { name: "min_score", type: "number", required: false, description: "Minimum AI score threshold (reserved for future use, default: 8)" },
          { name: "match_count", type: "number", required: false, description: "Max candidates returned by vector search (default: 50)" },
          { name: "max_results", type: "number", required: false, description: "Top N predictions to evaluate (default: 20)" },
        ],
        responseExample: JSON.stringify({
          success: true,
          precision: 0.35,
          ...
          parameters: { similarity_threshold: 0.30, min_score: 8, match_count: 50, max_results: 20 },
          duration_seconds: 22,
        }, null, 2),
      ...
      },
      {
        ...
        description: "Runs a full parameter sweep across all combinations of thresholds, match counts, and max results. Generates embeddings once then evaluates every parameter combination. Has a 5-minute time limit and reports partial results if stopped early. Each experiment result is saved individually to matching_experiments.",
        auth: "API Key",
        notes: "Long-running function. Embeddings are generated once per prospect and cached. Search results are cached per (prospect, threshold, match_count) to minimize DB calls. Returns the best-performing parameter set by F1 score.",
        params: [
          { name: "thresholds", type: "number[]", required: false, description: "Similarity thresholds to test (default: [0.15, 0.20, 0.25, 0.30, 0.35, 0.40])" },
          { name: "match_counts", type: "number[]", required: false, description: "Match count values to test (default: [30, 50, 75, 100])" },
          { name: "max_results_options", type: "number[]", required: false, description: "Max results values to test (default: [10, 15, 20, 30])" },
        ],
        responseExample: JSON.stringify({
          success: true,
          ...
          stopped_early: false,
          best_parameters: { similarity_threshold: 0.25, match_count: 75, max_results: 20 },
          best_f1: 0.42,
